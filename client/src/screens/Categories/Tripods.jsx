...
const Tripods = ({ match }) => {
    ...
    const tripod = products.filter(x => x.category === 'tripods')
        ...
        <>
            ...
            <div className='home' style={{ padding: '10px', margin: '10px' }} >
                ...
                {loading ? <Loader /> : error ? <h3> {toast.error(error)} </h3>
                    :
                    <div className="row ">
                        {tripod.length === 0 ? <p className='text-center p-3 mt-5 mb-5 display-4 font-weight-bold'> tripod are not avalible :(</p> : tripod.map((product) => (
