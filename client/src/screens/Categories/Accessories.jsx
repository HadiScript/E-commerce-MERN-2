...
const Accessories = ({ match }) => {
    ...
    const accessories = products.filter(x => x.category === 'watch')
        ...
        <>
            ...
            <div className='home' style={{ padding: '10px', margin: '10px' }} >
                ...
                {loading ? <Loader /> : error ? <h3> {toast.error(error)} </h3>
                    :
                    <div className="row ">
                        {accessories.length === 0 ? <p className='text-center p-3 mt-5 mb-5 display-4 font-weight-bold'> accessories are not avalible :(</p> : accessories.map((product) => (
