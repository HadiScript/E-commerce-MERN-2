...
const Camera = ({ match }) => {
    ...
    const cameras = products.filter(x => x.category === 'cameras')
        ...
        <>
            ...
            <div className='home' style={{ padding: '10px', margin: '10px' }} >
                ...
                {loading ? <Loader /> : error ? <h3> {toast.error(error)} </h3>
                    :
                    <div className="row ">
                        {cameras.length === 0 ? <p className='text-center p-3 mt-5 mb-5 display-4 font-weight-bold'> cameras are not avalible :(</p> : cameras.map((product) => (
