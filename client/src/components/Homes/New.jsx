...
function New() {
    ...
    const NewProducts = products.filter(x => x.keyMarket === 'New')


    if (error) {
        toast.error(error)
    }
    return loading ? <Loader /> : NewProducts.length > 0 && (
        <HeaderContainer >
            <div className="body my-5 mx-1">
                <hr />

                <Alert variant="dark text-center">
                    <Alert.Heading><h2 className='font-weight-bold'>New </h2></Alert.Heading>
                </Alert>
