...
function Latest() {
    ...
    const LatestProducts = products.filter(x => x.keyMarket === 'Latest')

    if (error) {
        toast.error(error)
    }

    return loading ? <Loader /> : LatestProducts.length > 0 && (
        <HeaderContainer >

            <div className="body my-5 mx-1">
                <hr />
                <Alert variant="dark text-center">
                    <Alert.Heading><h2 className='font-weight-bold'>Latest</h2></Alert.Heading>
                </Alert>
